...
### v1.1.0
- GitNexus 代码分析工具集成（impact/context/blast-radius）
- 更多 wiki resource 模板（concepts://, decisions://）
- pipeline 长任务异步后台执行 + 进度轮询

### v1.2.0
- MCP Tasks API 完整支持（pipeline_run 返回 taskId，客户端轮询）
- 自定义 transport 中间件（日志/鉴权）
- 多项目并行会话支持
